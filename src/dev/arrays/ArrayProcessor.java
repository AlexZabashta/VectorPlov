...
public interface ArrayProcessor<T> {
    ...
    void addTo(T src, int srcPos, T dest, int destPos, int length);
    ...
    default SubArray<T> allocSubArray(int length) {
        return new SubArray<T>(alloc(length), length);
    }

    default SubArray<T> allocSubArray(int length, double initialValue) {
        return new SubArray<T>(alloc(length, initialValue), length);
    }
    ...
    static int checkLength(int a, int b, String errorFormat) {
        if (a != b) {
            throw new IllegalArgumentException(String.format(errorFormat, a, b));
        }
        return a;
    }

    default void addTo(SubArray<T> src, SubArray<T> dst) {
        int length = checkLength(src.length, dst.length, "(src.length = %d) != (dst.length = %d)");
        addTo(src.array, src.offset, dst.array, dst.offset, length);
    }
